...
const TodoList = () => {
  ...
  const [getTodo, { loading: lazyLoading, data: lazyData }] =
  ...
  const [updateTodo] = useMutation(UPDATE_TODO_FIELD, {
    onCompleted: () => refetch(),
  });
  ...
  const [isOnEdit, setEdit] = useState(false);
  ...
  const findTodoByTitle = () => {
    if (selectValue === "" || title === "") return false;

    setInitialQuery(false);
    getTodo({ variables: { user_id: selectValue, title: `%${title}%` } });
    setTitle("");
    setSelectValue("");
  };
  ...
  const buttonAddClick = () => {
    ...
    addTodo({ variables: { objects: { id_user: selectValue, title: title } } });
    setTitle("");
    setSelectValue("");
  };
  ...
  /* Feature: Change Todo Status */

  const buttonChangeStatusClick = (id, status) => {
    updateTodo({ variables: { id: id, update: { is_done: !status } } });
    setInitialQuery(true);
  };

  /* Feature : Edit Todo */

  const buttonEditOnClick = (id, title) => {
    setEdit(true);
  ...
  const changeButtonClick = () => {
    updateTodo({
      variables: { id: currentTodo.id, update: { title: currentTodo.title } },
    });
    resetFormEdit();
  };
      ...
      <div className="container">
          ...
          <ul className="todo-list js-todo-list">
            {data.todolist.map((todo) => (
              <Todo
                ...
                onDeleteItem={deleteButtonClick}
                onClickItem={buttonChangeStatusClick}
                onEditItem={buttonEditOnClick}
              />
            ))}
          </ul>
        ) : (
        ...
        {loading || lazyLoading ? (
          ...
          <form onSubmit={(e) => e.preventDefault()}>
            ...
            />
            <input type="button" value="Change" onClick={changeButtonClick} />
            <input type="button" value="Cancel" onClick={resetFormEdit} />
          </form>
        ) : (
          <form className="js-form" onSubmit={(e) => e.preventDefault()}>
            ...
            <input type="button" value="Add Todo" onClick={buttonAddClick} />
          </form>
        )}
      </div>
